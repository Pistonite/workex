...
fn emit_interface_bus(
    ...
    let bind_config_func = match linked_interface.map(|i| &i.name) {
        Some(linked_name) => cconcat![
            ...
            " */",
            cblock! {
                format!("export const {prefix}{name} = (handler: {linked_name}, resolve?: (_: {name}) => (void | Promise<void>)): WxProtocolBindConfig<{name}> => {{ return {{"),
                [
        ...
        None => cconcat![
            ...
            " */",
            cblock! {
                format!("export const {prefix}{name} = (): WxProtocolBindConfig<Record<string, never>> => {{ return {{"),
                [
                    format!("protocol: {},", quoted(protocol)),
