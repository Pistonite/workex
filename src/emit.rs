...
pub fn emit(pkg: &Package) -> Result<(), Error> {
    ...
    for interface in &pkg.interfaces {
        ...
        emit_interface_recv(interface, out_dir, &pkg.lib_path)?;
    }
...
/// Emit protocol.ts
fn emit_protocol(pkg: &Package) -> Result<(), Error> {
    let protocol = &pkg.protocol;
    let out_dir = &pkg.out_dir;

    if out_dir.exists() {
        std::fs::remove_dir_all(out_dir).change_context(Error::IO)?;
    }
    std::fs::create_dir_all(out_dir).change_context(Error::IO)?;
...
fn emit_interface_send(interface: &Interface, out_dir: &Path) -> Result<(), Error> {
    let imports = &interface.imports.send;

    let class_decl = cblock! {
        format!("export class {0}Client implements {0} {{", interface.name),
        [
            format!("private client: {}<typeof {PROTOCOL_CONST_NAME}, {FUNCTION_ID_ENUM_NAME}>", imports.workex_client_ident),
            "",
            cblock! {
...
fn emit_interface_recv(interface: &Interface, out_dir: &Path, lib_path: &str) -> Result<(), Error> {
    let bind_func = cblock! {
        ...
        [
            ...
            cblock! {
                format!("bindHost({PROTOCOL_CONST_NAME}, options, shouldHandle, (fId: {FUNCTION_ID_ENUM_NAME}, _payload: any[]) => {{"),
                [
                    cblock! {
                        "switch (fId) {",
    ...
    let output = cconcat![
        header(),
        format!("import {{ type WorkexBindOptions, bindHost, unhandled }} from \"{lib_path}\";"),
        format!(
            "import type {{ {} }} from \"./{}\";",
            interface.name, interface.filename
        ),
